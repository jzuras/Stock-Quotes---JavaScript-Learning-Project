// this charting code is from (combine 2 lines below)
 ...
 * @param { [ {X,Y} ] } data - an array of objects with X and Y fields.
 * Only the Y field (stock price) is used here.
 */
function setMinMaxY( data )
{
    let max = 0;
 ...
 * @param {number} canvasWidth - width of drawing canvas.
 * @param {string} timeValue - time of quote in format 'HH:MM:SS'
 * @returns {number} the x-pixel position for the provided time value.
 */
function getXPixel( canvasWidth, timeValue )
{
    // timeValue is this date format (X value) - 2023-04-05 09:30:00
    // pull time from date, convert to seconds to compute x 
    // value across 6.5 hours( 9: 30 AM 4 PM )
 ...
 * @param {number} canvasHeight - height of drawing canvas.
 * @param {number} stockPrice - price for the stock being charted.
 * @returns {number} the y-pixel position for the provided stock price.
 */
function getYPixel( canvasHeight, stockPrice )
{
 ...
 */
export function drawChart( chartCanvas, data )
{
